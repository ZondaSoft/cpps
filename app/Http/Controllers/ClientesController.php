...
class ClientesController extends Controller
{
    ...
    public function index($id = null, $direction = null)
    {
        ...
        if ($id == null) {
            ...
            $legajo = Vta001::find($id);
            if ($legajo == null) {
                $legajo = Vta001::WhereNull('baja')
                    ->Where('codigo', '>', 0)
                    ->orderBy('codigo')
                    ->first();      // first(); // find($id);     // dd($legajo);   // $legajo = new Vta001;
            }
        ...
        if ($direction == -1) {
            $legajo = Vta001::where('codigo', '<', $nrolegajo)
                ->Where('codigo', '>', 0)
                ->WhereNull('baja')
                ->orderBy('codigo', 'desc')
                ->first();
    ...
    public function store(Request $request)
    {
        ...
        $legajo = new Vta001();
        //$request->all();
        //$legajo = Vta001::create($request->all()); // massives assignments : all() -> onLy() // only('name','description')
    ...
    {
        $active = 1;
        //$legajos = Vta001::paginate(5);
        $legajos = Vta001::name($request->get('name'))
            ->where('codigo', '!=', null)
            ->WhereNull('baja')
            ->orderBy('codigo')
            ->paginate(10);
