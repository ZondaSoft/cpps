                              ...
                              <div class="row">
                                <div class="col m8 s8 input-field">
                                  <select id="matricula" name="matricula" 
                                      {{ $edicion?'enabled':'disabled' }}>
                                      <option value = "" @if ( old('obra',$legajo->matricula)  == "")  selected   @endif  >Seleccione un profesional</option>
                                      @foreach ($profesionales as $profesional)
                                        <option value = "{{ $profesional->mat_prov_cole  }}" @if ( old('profesional',$legajo->mat_prov_cole)  == $profesional->matricula)  selected   @endif  >{{ $profesional->mat_prov_cole }} - {{ $profesional->nom_ape }}</option>
                                      @endforeach
                                  </select>
